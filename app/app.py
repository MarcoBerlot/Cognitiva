...
def savePost():
    ...
    output = []
    for s in star.find():
        output.append({'Date' : s['Date'], 'Score' : s['Score']})
...
def IdeaDensity():
	text="Good Afternoon All,Just wanted to share that the IT team along with members of the Administrative Support team have been discussing printer issues at length.  We realized that the current printers couldn’t handle volume of printing that we were producing.  We have since ordered two new printers that are being installed within the next few days.  After the installation and testing has been completed the IT team will give further instructions on how to print to those new printers.  These new printers also have a few new features that we thought would be helpful to you all.  Demonstrations will be scheduled to give you an overview on how to use.  Look out for invites.Lastly, the third floor printer had an issue that we corrected today.  You may now print legal documents (8 ½ x 14 Paper).  The larger paper is no longer a default so please use the bypass to print 16 x 20. "
